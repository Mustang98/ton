...
struct Transaction {
  ...
  // Per-transaction attribution is wall-only: reading the thread CPU clock in
  // every VM/storage interval is more expensive than the smallest intervals.
  td::RealCpuTimer::Time time_tvm = td::RealCpuTimer::Time::real_only();
  td::RealCpuTimer::Time time_storage_stat = td::RealCpuTimer::Time::real_only();
