...
struct BocImportLoadProbeState {
  std::string name;
  std::string failure;
  std::vector<std::string> *events{nullptr};
  unsigned loads{0};
  // -1 delegates to the wrapped cell; 0/1 deliberately overrides it for
  // malformed-wrapper ordering tests.
  int virtualized{-1};
};

class BocImportLoadProbe final : public Cell {
 public:
  static Ref<Cell> create(Ref<Cell> view, std::shared_ptr<BocImportLoadProbeState> state) {
    return Ref<BocImportLoadProbe>{true, std::move(view), std::move(state)};
  }

  BocImportLoadProbe(Ref<Cell> view, std::shared_ptr<BocImportLoadProbeState> state)
  ...
  td::Result<LoadedCell> load_cell() const override {
    ++state_->loads;
    if (state_->events) {
      state_->events->push_back(state_->name);
    }
    if (!state_->failure.empty()) {
      return td::Status::Error(state_->failure);
    }
    return view_->load_cell();
  }

  bool is_virtualized() const override {
    return state_->virtualized < 0 ? view_->is_virtualized() : state_->virtualized != 0;
  }
 ...
 private:
  ...
  Ref<Cell> view_;
  std::shared_ptr<BocImportLoadProbeState> state_;
};

struct BocImportCycleCellState {
  Ref<DataCell> loaded;
  unsigned loads{0};
};

class BocImportCycleCell final : public Cell {
 public:
  static Ref<Cell> create(Ref<Cell> claim, std::shared_ptr<BocImportCycleCellState> state) {
    return Ref<BocImportCycleCell>{true, std::move(claim), std::move(state)};
  }

  BocImportCycleCell(Ref<Cell> claim, std::shared_ptr<BocImportCycleCellState> state)
      : claim_(std::move(claim)), state_(std::move(state)) {
  }
  ...
  td::Result<LoadedCell> load_cell() const override {
    ++state_->loads;
    if (state_->loaded.is_null()) {
      return td::Status::Error("synthetic cycle cell is not initialized");
    }
  ...
  bool is_loaded() const override {
    return state_->loaded.not_null();
  }

  LevelMask get_level_mask() const override {
    return claim_->get_level_mask();
  }

 private:
  td::uint16 do_get_depth(td::uint32 level) const override {
    return claim_->get_depth(level);
  }

  const Hash do_get_hash(td::uint32 level) const override {
    return claim_->get_hash(level);
  }

  Ref<Cell> claim_;
  std::shared_ptr<BocImportCycleCellState> state_;
};

}  // namespace

TEST(TonDb, BocImportSingleProbeDuplicateDagAndRoots) {
  ...
  auto equal_root_a = CellBuilder{}.store_long(0xa7, 8).store_ref(equal_leaf_a).store_ref(equal_leaf_b).finalize_novm();
  auto equal_root_b = CellBuilder{}.store_long(0xa7, 8).store_ref(equal_leaf_b).store_ref(equal_leaf_a).finalize_novm();
  ASSERT_TRUE(equal_root_a.get() != equal_root_b.get());
  ASSERT_EQ(equal_root_a->get_hash(), equal_root_b->get_hash());

  BagOfCells distinct_objects;
  distinct_objects.set_roots({equal_root_a, equal_root_b, equal_leaf_b});
  distinct_objects.import_cells().ensure();

  BagOfCells canonical_objects;
  canonical_objects.set_roots({equal_root_a, equal_root_a, equal_leaf_a});
  canonical_objects.import_cells().ensure();

  for (int mode : get_serialization_modes()) {
    auto actual = distinct_objects.serialize_to_string(mode);
    auto expected = canonical_objects.serialize_to_string(mode);
    ASSERT_EQ(expected, actual);
    if (mode == 0) {
      ASSERT_EQ("b5ee9c7201010203000a0000010202a701010006123456", td::hex_encode(actual));
    } else if (mode == 31) {
      ASSERT_EQ("b5ee9c72e1010203000a0000010b150202a70101000612345615a57a8e", td::hex_encode(actual));
    }

    BagOfCells::Info info;
    ASSERT_EQ(static_cast<long long>(actual.size()), info.parse_serialized_header(actual));
    ASSERT_TRUE(info.valid && info.has_roots);
    ASSERT_EQ(2, info.cell_count);
    ASSERT_EQ(3, info.root_count);
    ASSERT_EQ(
        info.read_ref(reinterpret_cast<const unsigned char *>(actual.data()) + info.roots_offset),
        info.read_ref(reinterpret_cast<const unsigned char *>(actual.data()) + info.roots_offset + info.ref_byte_size));

    if (mode & BagOfCells::Mode::WithCacheBits) {
      ASSERT_TRUE(info.has_index && info.has_cache_bits);
      for (int i = 0; i < info.cell_count; ++i) {
        auto entry = info.read_offset(reinterpret_cast<const unsigned char *>(actual.data()) + info.index_offset +
                                      i * info.offset_byte_size);
        ASSERT_EQ(1u, entry & 1u);
      }
    }

    auto roundtrip_roots = deserialize_boc_multiple(actual);
    ASSERT_EQ(actual, serialize_boc(roundtrip_roots, mode));
  }
}

TEST(TonDb, BocImportSingleProbeLoadErrorOrderAndRetry) {
  auto left = CellBuilder{}.store_long(0x11, 8).finalize_novm();
  auto failing = CellBuilder{}.store_long(0x22, 8).finalize_novm();
  auto right = CellBuilder{}.store_long(0x33, 8).finalize_novm();
  std::vector<std::string> events;
  auto left_state = std::make_shared<BocImportLoadProbeState>(BocImportLoadProbeState{"left", "", &events});
  auto failing_state = std::make_shared<BocImportLoadProbeState>(
      BocImportLoadProbeState{"failing", "synthetic ordered load failure", &events});
  auto right_state = std::make_shared<BocImportLoadProbeState>(BocImportLoadProbeState{"right", "", &events});

  auto root = CellBuilder{}
                  .store_long(0x44, 8)
                  .store_ref(BocImportLoadProbe::create(left, left_state))
                  .store_ref(BocImportLoadProbe::create(failing, failing_state))
                  .store_ref(BocImportLoadProbe::create(right, right_state))
                  .finalize_novm();
  BagOfCells boc;
  boc.set_root(root);
  auto failed = boc.import_cells();
  ASSERT_TRUE(failed.is_error());
  ASSERT_EQ("error while importing a cell into a bag of cells: [Error : 0 : synthetic ordered load failure]",
            failed.message().str());
  ASSERT_EQ((std::vector<std::string>{"left", "failing"}), events);
  ASSERT_EQ(0u, right_state->loads);

  // A retry must discard both finalized indices and the in-progress marker.
  failing_state->failure.clear();
  events.clear();
  boc.import_cells().ensure();
  ASSERT_EQ((std::vector<std::string>{"left", "failing", "right"}), events);

  auto plain_root =
      CellBuilder{}.store_long(0x44, 8).store_ref(left).store_ref(failing).store_ref(right).finalize_novm();
  for (int mode : get_serialization_modes()) {
    ASSERT_EQ(serialize_boc(plain_root, mode), boc.serialize_to_string(mode));
  }
}

TEST(TonDb, BocImportSingleProbeRejectsCycleAtRegistration) {
  auto state_a = std::make_shared<BocImportCycleCellState>();
  auto state_b = std::make_shared<BocImportCycleCellState>();
  auto cell_a = BocImportCycleCell::create(CellBuilder{}.store_long(0xa1, 8).finalize_novm(), state_a);
  auto cell_b = BocImportCycleCell::create(CellBuilder{}.store_long(0xb2, 8).finalize_novm(), state_b);
  auto loaded_a = CellBuilder{}.store_long(0xc3, 8).store_ref(cell_b).finalize_novm();
  auto loaded_b = CellBuilder{}.store_long(0xd4, 8).store_ref(cell_a).finalize_novm();
  state_a->loaded = loaded_a;
  state_b->loaded = loaded_b;

  BagOfCells boc;
  boc.set_root(cell_a);
  auto result = boc.import_cells();
  ASSERT_TRUE(result.is_error());
  ASSERT_EQ("error while importing a cell into a bag of cells: cyclic cell graph", result.message().str());
  ASSERT_EQ(1u, state_a->loads);
  ASSERT_EQ(1u, state_b->loads);

  // Break the deliberately malformed ownership cycle before leaving the test.
  const_cast<DataCell *>(loaded_a.get())->reset_ref_unsafe(0, {}, false).clear();
  const_cast<DataCell *>(loaded_b.get())->reset_ref_unsafe(0, {}, false).clear();
  state_a->loaded.clear();
  state_b->loaded.clear();
}

TEST(TonDb, BocImportSingleProbeVirtualizedErrorPrecedesLoad) {
  std::vector<std::string> events;
  auto leaf = CellBuilder{}.store_long(0xe1, 8).finalize_novm();
  auto child_state = std::make_shared<BocImportLoadProbeState>(
      BocImportLoadProbeState{"virtualized-child", "must not be loaded", &events, 0, 1});
  auto child = BocImportLoadProbe::create(leaf, child_state);
  auto malformed_parent = CellBuilder{}.store_long(0xe2, 8).store_ref(std::move(child)).finalize_novm();
  ASSERT_TRUE(malformed_parent->is_virtualized());

  // The outer probe deliberately hides the malformed parent's flag so the
  // import reaches its child and exercises the established error precedence.
  auto root_state = std::make_shared<BocImportLoadProbeState>(BocImportLoadProbeState{"root", "", &events, 0, 0});
  BagOfCells boc;
  boc.set_root(BocImportLoadProbe::create(std::move(malformed_parent), root_state));
  auto result = boc.import_cells();
  ASSERT_TRUE(result.is_error());
  ASSERT_EQ("error while importing a cell into a bag of cells: cell has non-zero virtualization level",
            result.message().str());
  ASSERT_EQ((std::vector<std::string>{"root"}), events);
  ASSERT_EQ(0u, child_state->loads);
}

TEST(TonDb, BocImportSingleProbePreservesUsageAndMerkleContexts) {
  auto grandchild = CellBuilder{}.store_long(0x51, 8).finalize_novm();
  auto left = CellBuilder{}.store_long(0x52, 8).store_ref(grandchild).finalize_novm();
  auto right = CellBuilder{}.store_long(0x53, 8).finalize_novm();
  auto branch = CellBuilder{}.store_long(0x54, 8).store_ref(left).store_ref(right).finalize_novm();
  auto proof = CellBuilder::create_merkle_proof(branch);

  BagOfCells plain;
  plain.set_root(proof);
  plain.import_cells().ensure();

  auto usage_tree = std::make_shared<CellUsageTree>();
  std::vector<Cell::Hash> loads;
  usage_tree->set_cell_load_callback([&](const LoadedCell &loaded) { loads.push_back(loaded.data_cell->get_hash()); });
  BagOfCells observed;
  observed.set_root(UsageCell::create(proof, usage_tree->root_ptr()));
  observed.import_cells().ensure();

  ASSERT_EQ((std::vector<Cell::Hash>{proof->get_hash(), branch->get_hash(), left->get_hash(), grandchild->get_hash(),
                                     right->get_hash()}),
            loads);
  for (int mode : get_serialization_modes()) {
    ASSERT_EQ(plain.serialize_to_string(mode), observed.serialize_to_string(mode));
  }

  auto root_id = usage_tree->root_id();
  auto branch_id = usage_tree->get_child(root_id, 0);
  auto left_id = usage_tree->get_child(branch_id, 0);
  auto right_id = usage_tree->get_child(branch_id, 1);
  auto grandchild_id = usage_tree->get_child(left_id, 0);
  ASSERT_TRUE(usage_tree->is_loaded(root_id));
  ASSERT_TRUE(usage_tree->is_loaded(branch_id));
