...
TEST(VM, DataCellLiveRefSlots) {
  const auto cells_before = vm::DataCell::get_total_data_cells();
  {
    ...
    for (unsigned count = 0; count <= children.size(); ++count) {
      ...
      {
        auto consumed_children = children;
        std::array<int, vm::CellTraits::max_refs> owned_refcounts;
        for (unsigned i = 0; i < children.size(); ++i) {
          owned_refcounts[i] = children[i]->get_refcnt();
        }
        auto parent = vm::DataCell::create_consume({}, 0, {consumed_children.data(), count}, false).move_as_ok();
        ASSERT_EQ(parent->get_refs_cnt(), count);
        for (unsigned i = 0; i < children.size(); ++i) {
          ASSERT_EQ(i < count, consumed_children[i].is_null());
          ASSERT_EQ(children[i]->get_refcnt(), owned_refcounts[i]);
          if (i < count) {
            ASSERT_TRUE(parent->get_ref_raw_ptr(i) == children[i].get());
          } else {
            ASSERT_TRUE(consumed_children[i].get() == children[i].get());
          }
        }
      }
      for (unsigned i = 0; i < children.size(); ++i) {
    ...
    std::array<td::Ref<vm::Cell>, vm::CellTraits::max_refs + 1> too_many;
    for (auto& child : too_many) {
      child = children.front();
    }
    ASSERT_TRUE(vm::DataCell::create_consume({}, 0, too_many, false).is_error());
    for (const auto& child : too_many) {
      ASSERT_TRUE(child.get() == children.front().get());
    }

    std::array<unsigned char, 1> invalid_special_data{0};
    auto invalid_special_ref = children.front();
    ASSERT_TRUE(vm::DataCell::create_consume(td::Slice{invalid_special_data.data(), invalid_special_data.size()}, 8,
                                             td::mutable_span(&invalid_special_ref, 1), true)
                    .is_error());
    ASSERT_TRUE(invalid_special_ref.get() == children.front().get());
...
class CellCreateAccounting final : public vm::VmStateInterface {
 public:
  void register_cell_create() override {
    ++creates;
  }

  void register_new_cell(td::Ref<vm::DataCell>& cell) override {
    ++registrations;
    hashes.push_back(cell->get_hash());
  }

  int creates{0};
  int registrations{0};
  std::vector<vm::CellHash> hashes;
};

td::Ref<vm::DataCell> create_pruned_branch_with_builder(const td::Ref<vm::Cell>& cell, td::uint32 new_level,
                                                        td::uint32 virt_level = vm::Cell::max_level) {
  auto level_mask = cell->get_level_mask().apply(virt_level);
  auto level = level_mask.get_level();
  CHECK(new_level >= level + 1);
  vm::CellBuilder builder;
  builder.store_long(static_cast<td::uint8>(vm::Cell::SpecialType::PrunnedBranch), 8);
  builder.store_long(level_mask.apply_or(vm::Cell::LevelMask::one_level(new_level)).get_mask(), 8);
  for (td::uint32 i = 0; i <= level; ++i) {
    if (level_mask.is_significant(i)) {
      builder.store_bytes(cell->get_hash(i).as_slice());
    }
  }
  for (td::uint32 i = 0; i <= level; ++i) {
    if (level_mask.is_significant(i)) {
      builder.store_long(cell->get_depth(i), vm::Cell::depth_bytes * 8);
    }
  }
  return builder.finalize(true);
}

void assert_data_cells_exact(const td::Ref<vm::DataCell>& expected, const td::Ref<vm::DataCell>& actual) {
  ASSERT_EQ(expected->get_bits(), actual->get_bits());
  ASSERT_EQ(expected->get_refs_cnt(), actual->get_refs_cnt());
  ASSERT_EQ(expected->is_special(), actual->is_special());
  ASSERT_EQ(static_cast<int>(expected->special_type()), static_cast<int>(actual->special_type()));
  ASSERT_EQ(expected->get_level_mask().get_mask(), actual->get_level_mask().get_mask());
  const auto byte_size = static_cast<std::size_t>((expected->get_bits() + 7) / 8);
  const td::Slice expected_data{expected->get_data(), byte_size};
  const td::Slice actual_data{actual->get_data(), byte_size};
  ...
  ASSERT_TRUE(expected->serialize() == actual->serialize());
  ASSERT_TRUE(vm::std_boc_serialize(expected, 31).move_as_ok().as_slice() ==
              vm::std_boc_serialize(actual, 31).move_as_ok().as_slice());
}

}  // namespace

TEST(VM, CellBuilderDestructiveFinalizeConsumesChildReferences) {
  auto child = vm::CellBuilder{}.store_long(0x51, 8).finalize_novm();
  const auto base_refcount = child->get_refcnt();
  vm::CellBuilder builder;
  builder.store_long(0xa5, 8).store_ref(child);
  ASSERT_EQ(child->get_refcnt(), base_refcount + 1);
  auto parent = builder.finalize_novm();
  ASSERT_EQ(child->get_refcnt(), base_refcount + 1);
  ASSERT_TRUE(parent->get_ref_raw_ptr(0) == child.get());
  parent.clear();
  ASSERT_EQ(child->get_refcnt(), base_refcount);
}

TEST(VM, CellBuilderDirectConstructionPreservesBytesFailureAndVmAccounting) {
  auto child = vm::CellBuilder{}.store_long(0x52, 8).finalize_novm();
  std::array<unsigned char, 3> data{0xa5, 0x5a, 0xc3};

  CellCreateAccounting builder_accounting;
  td::Ref<vm::DataCell> expected;
  {
    vm::CellBuilder builder;
    builder.store_bytes(data.data(), data.size()).store_ref(child);
    vm::VmStateInterface::Guard guard{&builder_accounting};
    expected = builder.finalize();
  }

  auto direct_ref = td::Ref<vm::Cell>{child};
  CellCreateAccounting direct_accounting;
  td::Ref<vm::DataCell> actual;
  {
    vm::VmStateInterface::Guard guard{&direct_accounting};
    actual = vm::CellBuilder::create_data_cell(td::Slice{data.data(), data.size()}, data.size() * 8,
                                               td::mutable_span(&direct_ref, 1));
  }
  ASSERT_TRUE(direct_ref.is_null());
  assert_data_cells_exact(expected, actual);
  ASSERT_EQ(1, builder_accounting.creates);
  ASSERT_EQ(1, builder_accounting.registrations);
  ASSERT_EQ(builder_accounting.creates, direct_accounting.creates);
  ASSERT_EQ(builder_accounting.registrations, direct_accounting.registrations);
  ASSERT_TRUE(builder_accounting.hashes == direct_accounting.hashes);

  std::array<unsigned char, 1> invalid_special_data{0};
  auto failed_ref = td::Ref<vm::Cell>{child};
  auto* failed_ref_ptr = failed_ref.get();
  const auto failed_refcount = child->get_refcnt();
  CellCreateAccounting failure_accounting;
  bool threw = false;
  {
    vm::VmStateInterface::Guard guard{&failure_accounting};
    try {
      static_cast<void>(
          vm::CellBuilder::create_data_cell(td::Slice{invalid_special_data.data(), invalid_special_data.size()}, 8,
                                            td::mutable_span(&failed_ref, 1), true));
    } catch (const vm::CellBuilder::CellWriteError&) {
      threw = true;
    }
  }
  ASSERT_TRUE(threw);
  ASSERT_TRUE(failed_ref.get() == failed_ref_ptr);
  ASSERT_EQ(failed_refcount, child->get_refcnt());
  ASSERT_EQ(1, failure_accounting.creates);
  ASSERT_EQ(0, failure_accounting.registrations);
}

TEST(VM, CellBuilderDirectPrunedBranchMatchesBuilderExactly) {
  auto leaf = vm::CellBuilder{}.store_long(0x123456, 24).finalize_novm();
  td::Ref<vm::Cell> source = vm::CellBuilder{}.store_long(0xa5, 8).store_ref(leaf).finalize_novm();

  for (td::uint32 new_level = 1; new_level <= vm::Cell::max_level; ++new_level) {
    CellCreateAccounting expected_accounting;
    td::Ref<vm::DataCell> expected;
    {
      vm::VmStateInterface::Guard guard{&expected_accounting};
      expected = create_pruned_branch_with_builder(source, new_level);
    }

    CellCreateAccounting actual_accounting;
    td::Ref<vm::DataCell> actual;
    {
      vm::VmStateInterface::Guard guard{&actual_accounting};
      actual = vm::CellBuilder::do_create_pruned_branch(source, new_level);
    }
    assert_data_cells_exact(expected, actual);
    ASSERT_EQ(static_cast<td::uint8>(vm::Cell::SpecialType::PrunnedBranch), actual->get_data()[0]);
    ASSERT_EQ(expected->get_data()[1], actual->get_data()[1]);
    ASSERT_EQ(1, actual_accounting.creates);
    ASSERT_EQ(1, actual_accounting.registrations);
    ASSERT_EQ(expected_accounting.creates, actual_accounting.creates);
    ASSERT_EQ(expected_accounting.registrations, actual_accounting.registrations);
    ASSERT_TRUE(expected_accounting.hashes == actual_accounting.hashes);

    if (new_level != vm::Cell::max_level) {
      source = vm::CellBuilder{}.store_long(new_level, 8).store_ref(actual).finalize_novm();
    }
  }
}
