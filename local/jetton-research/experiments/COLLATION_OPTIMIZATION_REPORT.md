...
**Research date:** 2026-08-05 through 2026-08-10 UTC
**Repository:** `/home/vallas/ton`
**Branch:** `fast-collators`
**Recorded Git HEAD:** `9f29b8b6`
**Status:** Experimental result, reproduced twice below 100 ms at `2x`; follow-up `4x` capacity measurements included; not yet a production-ready patch set
...
A 2026-08-10 follow-up raised the byte/gas-family limits to `4x`, removed an
experimental hardcoded 40% external-intake cap, and measured 650, 700, and 760
requests/s. The uncapped 700 run completed all 20,911 liteserver-acknowledged
requests, sustained a zero queue slope, and had 184.927/274.060/292.122 ms
productive-block mean/P95/max collation. The 760 run was not sustainable: its
internal queue grew by 69.6 messages/s and productive collation reached
282.733/530.857/644.781 ms, causing six deadline overloads. Section 13.9 records
the complete `4x` audit and explains why 700 is a short-run pass while 760 is a
fail.

The cap was subsequently restored as the validator-engine option
`--external-message-intake-limit`. It defaults to `0.4`, accepts a fraction in
`[0, 1]`, and is disabled with `0`; the `4x` uncapped measurements used the
equivalent of `--external-message-intake-limit 0`.
...
### 13.9 Four-times-limit capacity and external-intake audit

The final follow-up asked how far the optimized one-shard collator could scale
when the byte/gas-family limits were raised from `2x` to `4x`. These runs are a
capacity extension, not replacements for the matched `2x` optimization result.
They used:

- the same 45M-account generated state and manifest;
- 10,000 active `WalletSpam` senders, deterministic RNG seed 1, and
  `init_mode=0`;
- one unsplit basechain shard and one validator/full node;
- Simplex v2 with a 400 ms target slot;
- the first slot of every four-slot leader window forced empty;
- 8 validator-engine actor threads and 16 parallel execution workers;
- `block_limit_mul=4` and `gas_limit_mul=4`;
- a 30-second offered-load interval, 5-second warmup, and 15-second drain;
- the same 19-feature optimized environment from section 14, except
  `TON_SIM_EXECUTION_THREADS=16` instead of 32.

The effective configured soft/hard limits were 4,194,304/8,388,608 bytes,
40,000,000/80,000,000 gas, and 20,000/40,000 LT delta. The uncapped binary also
contained the transport headroom required for blocks above the previous 4 MiB
full-node ceiling: an 8 MiB full-node block limit and a 32 MiB overlay FEC limit.

#### 13.9.1 The removed 40% intake cap

The first two `4x` probes still contained this experimental parallel-collator
guard:

```cpp
constexpr double ext_load_fraction_limit = 0.4;

if (!full && block_limit_status_->load_fraction(block::ParamLimits::cl_soft) >=
                 ext_load_fraction_limit) {
  full = true;
}
```

It stopped accepting additional external messages after estimated block usage
reached 40% of the normal soft limit, even though the block itself was not full.
The intention was to reserve capacity for the three internal transactions that
follow each WalletSpam external. In practice it imposed a separate throughput
ceiling, delayed otherwise valid externals, and amplified external-pool drops and
wallet `seqno` holes. It was not part of upstream TON block-limit policy.

Commit `9f29b8b6` removed the constant and condition for the uncapped measurements.
The threshold was subsequently restored as
`validator-engine --external-message-intake-limit <fraction>` instead of a
compile-time constant. It defaults to `0.4`; `0` disables the separate threshold
and reproduces the uncapped behavior. Internal messages generated by accepted
externals remain subject to the ordinary block-limit machinery; consequently,
the final block can progress beyond the soft limit toward the configured hard
limit.

#### 13.9.2 Binary identity and run index

| Variant | `validator-engine` SHA-256 | Source state |
|---|---|---|
| Capped control | `20acf5a9927150a6a9e1ef7513b1dbd446a23768865f5815a77bc4f54e439dcb` | cleaned optimized stack with 40% cap |
| Uncapped | `9f1577da9784884058409e345e237c514e0de0335f8d78fd6634250e4981579b` | cap removed; removal later committed as `9f29b8b6` |

Both variants used the same sender binary:

```text
jetton-simulator:
  fc737ec367ff5a6861a46ae6567bda84ea9f8af02f7f354b3da6fea7ae62cbd2
```

Run directories, relative to `local/jetton-research/experiments/runs/`:

| Variant | Offered rate | Run directory |
|---|---:|---|
| Capped | 760/s | `audit-4m-optimized-10k-760-20260810-091018` |
| Capped | 650/s | `audit-4m-optimized-10k-650-20260810-093519` |
| Uncapped | 650/s | `audit-4m-no-intake-cap-10k-650-20260810-101314` |
| Uncapped | 700/s | `audit-4m-no-intake-cap-10k-700-20260810-113142` |
| Uncapped | 760/s | `audit-4m-no-intake-cap-10k-760-20260810-112444` |

#### 13.9.3 Admission and end-to-end completion

`Steady jTPS` counts successful TX3 transactions inside the cropped measurement
window. It can exceed the offered rate when warmup backlog completes after the
window starts. `Eventual completions / 30 s` divides all TX3 completions,
including drain completions, by the 30-second offer duration. Neither value alone
establishes sustainability; queue slope, deadlines, and acknowledged-message
loss must also remain bounded.

| Variant | Offered | Acknowledged | Send errors | Ack but no TX1 | Eventual TX3 | Eventual/30 s | Steady jTPS | Steady raw TPS |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Capped | 650/s | 19,243 | 257 | 593 | 18,650 | 621.7 | 637.885 | 2,551.542 |
| Capped | 760/s | 21,168 | 1,632 | 1,383 | 19,785 | 659.5 | 662.882 | 2,651.526 |
| Uncapped | 650/s | 19,471 | 29 | **0** | 19,471 | 649.0 | 663.882 | 2,656.646 |
| Uncapped | 700/s | 20,911 | 89 | **0** | 20,911 | 697.0 | 716.750 | 2,866.999 |
| Uncapped | 760/s | 22,386 | 414 | 287 | 22,099 | 736.6 | 636.233 | 2,593.898 |

Every included request in every run eventually completed TX1, TX2, TX3, and
TX4 with zero aborts. `Ack but no TX1` is therefore external-pool loss before
on-chain execution, not a failed jetton transaction.

The remaining `send errors` all have the liteserver category prefix
`cannot apply external message to current state ... cannot run message on
account`. They begin only after sender ID reuse starts. The generator uses each
of the 10,000 senders once per shuffled epoch and optimistically increments its
cached `seqno`; at 650, 700, and 760 requests/s, reuse begins after approximately
15.38, 14.29, and 13.16 seconds. If a preceding external has not reached chain
state, the next `seqno` fails the WalletSpam TVM admission check. This is a
10,000-wallet load-generator effect and is reported separately from collator
throughput.

Removing the cap eliminated acknowledged-message loss at 650 and 700 and cut
the 760 unmatched count from 3,015 to 701. It did not make 760 sustainable,
because the bottleneck moved from artificial external admission to block size,
internal-message backlog, and collation time.

#### 13.9.4 Productive-block size and collation

All size values below are decimal MB and include accepted productive blocks
only. Empty forced-first-slot candidates remain part of queue/cadence analysis
but are excluded from per-productive-block means and percentiles.

| Variant | Rate | Productive/empty | TX/block mean/P95/max | Block MB mean/P95/max | Collated MB mean/P95/max | Collation ms mean/P95/max |
|---|---:|---:|---:|---:|---:|---:|
| Capped | 650 | 47/15 | 1,357.1 / 1,642.4 / 1,776 | 2.601 / 3.132 / 3.364 | 2.040 / 2.494 / 2.651 | 170.878 / 213.914 / 234.340 |
| Capped | 760 | 45/15 | 1,472.8 / 1,720.8 / 2,036 | 2.979 / 4.068 / 4.082 | 2.361 / 3.376 / 3.686 | 202.371 / 335.369 / 362.710 |
| Uncapped | 650 | 48/16 | 1,383.4 / 2,029.3 / 2,032 | 2.646 / 3.875 / 3.880 | 2.029 / 3.293 / 3.315 | 173.969 / 271.612 / 291.991 |
| Uncapped | 700 | 48/16 | 1,492.9 / 2,075.7 / 2,119 | 2.883 / 3.970 / 4.122 | 2.211 / 3.357 / 3.388 | 184.927 / 274.060 / 292.122 |
| Uncapped | 760 | 40/13 | 1,621.0 / 2,206.3 / 2,264 | 3.675 / 5.777 / 5.843 | 2.801 / 4.473 / 4.514 | 282.733 / 530.857 / 644.781 |

The 700 run used the larger block allowance without approaching the 400 ms slot
deadline: productive P95/max collation were 274.060/292.122 ms. The 760 run
crossed the cadence boundary. Productive P95 was 530.857 ms, maximum was 644.781
ms, and only 40 productive candidates were accepted in the steady interval
instead of 48 at 650 and 700. Larger individual blocks could not compensate for
the lost productive slots.

#### 13.9.5 Queue, overload, and pipeline behavior

| Variant | Rate | Queue first/last/max | Queue slope | Block-limit overloads | Deadline overloads | Same-block TX1-TX4 | Inclusion P50/P99 |
|---|---:|---:|---:|---:|---:|---:|---:|
| Capped | 650 | 0 / 0 / 1 | 0/s | 0 | 0 | 100.0% | 1,362.9 / 1,949.5 ms |
| Capped | 760 | 0 / 0 / 646 | 0/s | 7 | 2 | 89.4% | 1,461.3 / 2,519.7 ms |
| Uncapped | 650 | 32 / 32 / 64 | 0/s | 15 | 0 | 93.7% | 1,221.5 / 2,082.7 ms |
| Uncapped | 700 | 0 / 0 / 160 | 0/s | 16 | 0 | 92.7% | 1,279.1 / 2,115.9 ms |
| Uncapped | 760 | 0 / 1,739 / 2,204 | **+69.6/s** | 19 | **6** | 40.6% | 1,718.6 / 6,637.4 ms |

At 700, all steady TX stage counts were equal at 17,915. The queue returned to
zero, no deadline fired, and all 20,911 acknowledged externals appeared on chain.
This is a short-run stability pass at approximately 700 completed jetton
transfers/s.

At 760, the steady stage counts diverged as work moved into later blocks:
...
The internal queue grew by 1,739 messages during the steady interval, peaked at
2,204, and required the post-load drain to clear. Six deadline overloads and a
6.64-second P99 inclusion latency confirm that 760 is not a sustainable point,
even though all 22,099 included requests eventually completed after load stopped.

#### 13.9.6 Capacity conclusion

For this exact one-validator, one-shard, 45M-state, 10k-wallet, forced-empty-slot
configuration:

- 650 requests/s is stable with substantial timing margin;
- 700 requests/s passes the 30-second short-run criteria with zero queue growth,
  zero acknowledged-message loss, and zero deadline overloads;
- 760 requests/s fails because larger blocks exceed the slot-time budget, reduce
  productive candidate cadence, and accumulate internal messages;
- therefore, the measured short-run sustainable boundary lies in `[700, 760)`
  requests/s and should not be narrowed or called production-safe without a
  longer soak and multi-validator propagation/validation measurements.

The uncapped mode is retained as a runtime setting because it exposes the real
limiting behavior without requiring a rebuild. The default remains the 40% cap;
neither mode removes the need for ordinary soft/hard block limits, collation
deadlines, queue monitoring, or split policy.
...
The final source has no hardcoded external-intake fraction in the collator. The
equivalent threshold is controlled by validator-engine option
`--external-message-intake-limit`, defaults to `0.4`, and can be varied without
rebuilding. Pass `0` when reproducing the uncapped section 13.9 runs; in that
mode, external intake is bounded by ordinary block-limit status and the collation
deadline.
...
### 15.8 Reproduce the uncapped `4x` 700 requests/s run

Use the validator binary with the runtime external-intake option and the raised
transport headroom described in section 13.9. Disable the default cap with
`--external-message-intake-limit 0`. The recorded run used 16 transaction
workers, not the 32-worker setting used for the final `2x` optimization audit.
...
cmd=(
  ...
  --build-dir /home/vallas/ton/build-clang-release
  --state-manifest
  /home/vallas/ton/_local/jetton-research/generated-db-45m/state/manifest.json
  --workdir
  /home/vallas/ton/local/jetton-research/experiments/runs/reproduce-4m-uncapped-700
  --pool-size 10000
  --init-mode 0
  --rate 700
  --duration 30
  ...
  --node-threads 8
  --node-arg=--external-message-intake-limit=0
  --base-port 13700
)

for value in "${x4_env[@]}"; do
  cmd+=(--node-env "$value")
done
...
Use a free `--base-port` range and a fresh `--workdir` for each repetition. A
matching short-run pass requires all acknowledged requests to reach TX1-TX4,
zero queue slope, zero deadline overloads, one shard, no parser/join failures,
and productive collation below the 400 ms slot target. The historical run's
measured-window jTPS of 716.750 is a warmup/window-boundary effect; compare the
20,911 eventual completions over the 30-second offer duration (697.0/s) as well
as the steady-window counters.
...
The later `4x` extension removed the artificial 40% external-intake ceiling and
separated admission limits from the true capacity boundary. At 700 requests/s,
all 20,911 acknowledged requests completed, productive collation was
184.927/274.060/292.122 ms mean/P95/max, and the internal queue had zero net
growth. At 760 requests/s, productive collation rose to
282.733/530.857/644.781 ms, six deadlines fired, and the queue grew by 69.6
messages/s. The supported conclusion is therefore a short-run one-shard `4x`
capacity of at least approximately 700 jetton transfers/s but less than 760 for
this host and workload, not a production network limit.
